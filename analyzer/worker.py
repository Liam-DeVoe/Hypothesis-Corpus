...
class Worker(Process):
    ...
    def __init__(self, worker_id: int, task_queue: Queue, result_queue: Queue, 
                 db_path: str, docker_image: str = "pbt-analyzer:latest"):
        ...
        while True:
            try:
                ...
                result = self._process_repository(
                    work_item, db, test_runner, analyzer
                )
    ...
    def _process_repository(self, work_item: WorkItem, db: Database, 
                           test_runner: TestRunner, analyzer: PropertyAnalyzer) -> Dict:
        """Process a single repository."""
        try:
            # Add repository to database if not already there
            if work_item.repo_id is None:
                owner, name = work_item.repo_name.split('/')
                work_item.repo_id = db.add_repository(owner, name, 
                                                      f"https://github.com/{work_item.repo_name}")
            ...
            if 'error' in results:
                db.update_repository_status(work_item.repo_id, 'failed', results['error'])
            ...
            for node_id, test_results in results.items():
                ...
                test_id = db.add_test(
                    work_item.repo_id, node_id, file_path, 
                ...
                if 'source_code' in test_results:
                    ...
                    enhanced_results = analyzer.analyze_source(test_results['source_code'])
                    test_results.update(enhanced_results)
                    
                    # Store in database
                    db.add_test_code(test_id, test_results['source_code'], 
                ...
                for gen_name, count in test_results.get('generators', {}).items():
                    if gen_name in ['composite', 'custom_strategies']:
                        db.add_generator_usage(test_id, gen_name, 1, 
...
class WorkerPool:
    ...
    def __init__(self, num_workers: int = 4, db_path: str = "data/analysis.db",
    ...
    def start(self):
        ...
        for i in range(self.num_workers):
            worker = Worker(i, self.task_queue, self.result_queue, 
                          self.db_path, self.docker_image)
            worker.start()
    ...
    def wait_for_completion(self, expected_count: int, timeout: int = 3600) -> List[Dict]:
        ...
        while completed < expected_count:
            if time.time() - start_time > timeout:
                logger.error(f"Timeout waiting for tasks (completed {completed}/{expected_count})")
                break
            
            result = self.get_result(timeout=1)
            if result:
                completed += 1
                logger.info(f"Completed {completed}/{expected_count}: {result['repo_name']} "

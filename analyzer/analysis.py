...
class PropertyAnalyzer:
    ...
    def _build_strategy_patterns(self) -> Dict[str, re.Pattern]:
        ...
        strategies = [
            ...
            'emails', 'urls', 'ip_addresses', 'complex_numbers',
            'fractions', 'decimals', 'characters', 'from_regex',
            'from_type', 'deferred', 'data', 'builds', 'fixed_dictionaries',
            'recursive', 'composite', 'shared', 'runner'
        ]
    ...
    def _build_property_patterns(self) -> Dict[str, List[re.Pattern]]:
        ...
        return {
            'mathematical': [
                re.compile(r'\b(?:commutative|associative|distributive|identity)\b', re.I),
            ...
            'oracle': [
                re.compile(r'reference.*implementation|implementation.*reference', re.I),
    ...
    def extract_generators(self, source: str) -> Dict[str, int]:
        ...
        if '@composite' in source or '@st.composite' in source:
            generators['composite'] = source.count('@composite') + source.count('@st.composite')
        
        # Check for custom strategies
        custom_pattern = re.compile(r'def\s+\w+\s*\([^)]*\)\s*->\s*(?:st\.)?SearchStrategy')
    ...
    def detect_patterns(self, source: str) -> Dict[str, bool]:
        ...
        # Check for fixtures
        patterns['uses_fixtures'] = '@pytest.fixture' in source or 'def fixture' in source
        ...
        patterns['uses_database'] = any(db in source.lower() for db in ['database', 'db', 'sql', 'orm'])
        patterns['uses_files'] = any(op in source for op in ['open(', 'read(', 'write('])
        patterns['uses_network'] = any(net in source.lower() for net in ['http', 'request', 'socket', 'url'])

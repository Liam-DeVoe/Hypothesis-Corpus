...
@click.option('--sample', '-s', is_flag=True, help='Run sample test with MarkCBell/bigger')
...
def main(dataset: str, config: str, workers: int, sample: bool, limit: int, docker_image: str):
        ...
        with WorkerPool(
            ...
            for i in range(len(work_items)):
                ...
                if result:
                    if result['success']:
                        successful += 1
                        console.print(f"✅ {result['repo_name']}: [green]Success[/green]")
                    else:
                        failed += 1
    ...
    for feature in stats['feature_usage']:
        console.print(f"  • {feature['feature_name']}: {feature['total_uses']} uses in {feature['test_count']} tests")
    
    console.print()
    console.print(f"[green]✨ View results in the dashboard: streamlit run dashboard.py[/green]")

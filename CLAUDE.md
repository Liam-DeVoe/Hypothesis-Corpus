# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

This is the Hypothesis Corpus system, which analyzes Hypothesis test patterns across GitHub repositories. It uses Docker containers for isolated test execution and provides real-time visualization of analysis results.

## Setup

```bash
pip install -r requirements.txt
just image  # Build Docker image for analysis
```

Requires `analysis/secrets.json` with API tokens (see `analysis/secrets.json.example`):
```json
{
  "claude_code_oauth_token": "your-token-here",
  "github_token": "your-token-here"
}
```

## Commands

All commands are available via `just` (uses `python3.13`). You can also run `python run.py <command>` directly.

### Workflow: Collect → Install → Experiment → Task
```bash
# Step 1: Collect repositories from GitHub
just collect

# Step 2: Install repos and collect test node IDs (runs in Docker)
just install --limit 10
just install --limit 10 --debug  # Show container logs
just install --repo owner/repo --overwrite  # Re-install a single repo

# Step 3: Run experiments on valid repos
just experiment --workers 4
just experiment -e runtime -e facets  # Specific experiments
just experiment --repo owner/repo --overwrite  # Single repo

# Step 4: Run post-experiment tasks
just task run cluster
just task clear --task-name cluster
```

### Dashboard
```bash
just dashboard
just dashboard --db-path test.db --port 8502
```

### Utilities
```bash
# Rebuild Docker image after changes
just image

# Mark a repository as invalid
just mark_invalid owner/repo autogenerated_tests

# Generate sankey diagram visualization
just sankey

# Check database contents
sqlite3 data/data.db ".tables"
sqlite3 data/data.db "SELECT full_name, status FROM core_repository LIMIT 5;"
```

## Architecture

### Data Flow
1. **collect**: Searches GitHub for repos using Hypothesis, stores in `core_repository` table. Uses file size bucketing to work around GitHub's 1000-result limit. Deduplicates repos via MinHash (Jaccard threshold 0.75).
2. **install**: Clones repos, runs Docker container to install deps and collect pytest node IDs via custom plugin, captures git commit hash, sets `status='valid'`. Aborts after 7 consecutive failures as a safeguard.
3. **experiment**: WorkerPool distributes valid repos across processes. Each worker clones the exact commit hash captured during install, copies experiment modules into a tar archive, executes in Docker container, parses results.json.
4. **task**: Runs post-experiment analysis (e.g., clustering) on the host machine (not Docker).
5. **dashboard**: Streamlit visualization reading from SQLite.

### Docker Container Execution
- Files packaged as tar archive and sent via Docker API (avoids Mac mount penalties)
- Container installs repo dependencies, then runs experiment via `runner.py`
- Network access required for pip installs
- Experiment modules copied into container: `runner.py`, `experiment.py`, `utils.py`, `{experiment_name}.py`
- A shared uv cache volume (`hypothesis-corpus-uv-cache`, 8GB limit) speeds up pip installs across containers
- `CLAUDE_CODE_OAUTH_TOKEN` env var passed through for the facets experiment

### Experiment System
Experiments run in Docker to analyze individual tests:
- Inherit from `Experiment` (analysis/experiments/experiment.py)
- Auto-register via `__init_subclass__` using `name` class attribute
- Implement: `get_schema_sql()`, `run(node_id)`, `store_to_database()`, `delete_data()`
- Optional: `run_repository()` for repo-level analysis, `only_canonical_nodes = True` to skip parametrized duplicates

Built-in: `runtime` (strategies/features), `facets` (Claude-generated summaries/patterns/domains)

Experiment modules must work both as package imports and standalone (in the container). Use the try/except pattern:
```python
try:
    from .experiment import Experiment
except ImportError:
    from experiment import Experiment
```

### Task System
Tasks run after experiments on the host machine:
- Inherit from `Task` (analysis/tasks/task.py)
- Auto-register via `__init_subclass__` using `name` class attribute
- Declare `follows = ["experiment_name"]` for dependencies
- Implement: `get_schema_sql()`, `run(db)`, `store_to_database()`, `delete_data()`

Built-in tasks:
- `aggregate_metrics`: Follows `runtime`. Precomputes per-node metrics for fast dashboard queries.
- `cluster`: Follows `facets`. Sentence-transformers embeddings + k-means, Claude for cluster naming.

### Canonical Parametrization
Tests with parametrize (`test_foo[0]`, `test_foo[1]`) are grouped by base name (`test_foo`). The first node in each group is marked canonical. The `facets` experiment uses `only_canonical_nodes = True` to avoid analyzing the same test logic multiple times.

### Database
Data is split across three SQLite files, ATTACHed to a single connection so all queries resolve table names automatically:
- `data/data.db`: Core tables, experiment summaries, tasks
- `data/data_test_cases.db`: `runtime_test_case` (large, can be downloaded separately)
- `data/data_minhashes.db`: `core_minhashes` (only needed for dedup reproducibility)

Experiments declare which db their tables live in via `get_schema_sql()` returning `dict[str, str]` keyed by db name (`"main"`, `"test_cases"`, etc.).

Singleton pattern via `get_database(path)`.

**Core tables (data.db):**
- `core_repository`: Collected repos (full_name, status, requirements, node_ids, other_node_ids, commit_hash)
- `core_node`: Test nodes (repo_id, node_id, canonical_parametrization, source_code, is_stateful)

**Core tables (data_minhashes.db):**
- `core_minhashes`: MinHash for deduplication

**Experiment tables** defined by `get_schema_sql()`:
- `runtime`: `runtime_summary` (data.db), `runtime_test_case` (data_test_cases.db)
- `facets`: `facets_repository`, `facets_nodes` (data.db)

**Task tables:**
- `aggregate_metrics`: `node_aggregate_metrics`
- `cluster`: `facets_cluster`, `facets_cluster_assignment`

**API:**
```python
from analysis.database import Database, get_database

db = Database(db_dir="data")           # Direct construction
db = get_database("data")              # Singleton per path (used in dashboard)
db.execute(query, params)
db.fetchone(query, params)
db.fetchall(query, params)
db.commit()
pd.read_sql_query("SELECT ...", db._conn)  # For pandas
```

## Adding New Experiments

1. Create class in `analysis/experiments/` inheriting from `Experiment`
2. Set `name` class attribute for auto-registration
3. Implement `get_schema_sql()`, `run(node_id)`, `store_to_database()`, `delete_data()`
4. Use try/except import pattern for standalone execution in containers
5. Rebuild Docker image if adding system dependencies

## Adding New Tasks

1. Create class in `analysis/tasks/` inheriting from `Task`
2. Set `name` class attribute for auto-registration
3. Implement `get_schema_sql()`, `run(db)`, `store_to_database()`, `delete_data()`
4. Set `follows = ["experiment_name"]` for dependencies
5. Export from `analysis/tasks/__init__.py`

### Dashboard
- **dashboard/Overview.py**: Streamlit entry point with page navigation
- **dashboard/shared.py**: Shared visualization utilities (histograms, KDE)
- **dashboard/pages/**: Clusters, Facets, corpus, coverage, settings, testcases, timing

## Key Files

- **run.py**: CLI entry point (click-based)
- **justfile**: Build automation (`just image`, `just experiment`, etc.)
- **analysis/worker.py**: Multiprocessing pool for experiments
- **analysis/test_runner.py**: Clones repo, sets up Docker, parses results
- **analysis/experiments/runner.py**: Entry point inside Docker container
- **analysis/collect/install_repos.py**: Orchestrates repo installation
- **analysis/collect/_install.py**: Runs inside Docker to install deps and collect nodes
- **analysis/database.py**: SQLite wrapper with singleton pattern
- **pytest_hypothesis_corpus/plugin.py**: Custom pytest plugin for observation collection (runs inside Docker)
- **sankey/sankey.py**: Sankey diagram generator for visualization
